...
function CanvasElement({ 
  ...
  const renderContent = () => {
    switch (element.type) {
      ...
      case 'AssetImage': {
        const imageSrc = 'imageSrc' in element.properties ? element.properties.imageSrc : undefined;
        const objectFit = 'objectFit' in element.properties ? element.properties.objectFit : 'contain';
        const tint = 'tint' in element.properties ? element.properties.tint : undefined;
        const assetPath = 'assetPath' in element.properties ? element.properties.assetPath : undefined;
        ...
        return (
          <div className="w-full h-full flex flex-col items-center justify-center bg-stone-700/50 border border-dashed border-stone-600 rounded pointer-events-none gap-1">
            <svg className="w-8 h-8 text-foreground-subtle opacity-50" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
            </svg>
            <span className="text-[10px] text-foreground-subtle opacity-60">
              {assetPath || 'No image'}
            </span>
          </div>
        );
