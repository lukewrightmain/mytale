...
function generateElementUI(element: UIElement, lines: string[], indent: number, isRoot: boolean = false): void {
  ...
  // Generate clean element ID
  const elementId = getCleanId(element, isRoot);
  const idPart = elementId ? ` #${elementId}` : '';
  ...
  // Add blank line before children if there are both properties and children
  ...
  for (let i = 0; i < element.children.length; i++) {
    generateElementUI(element.children[i], lines, indent + 1);
...
function generatePropertiesUI(props: ElementProperties, lines: string[], indent: number): void {
  ...
  if ('objectFit' in props && props.objectFit && props.objectFit !== 'contain') {
    lines.push(`${prefix}ObjectFit: ${props.objectFit};`);
  }
